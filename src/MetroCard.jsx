...
function MetroCard({ onEnter }) {
  ...
  // Which transit card front is currently active.
  const [selectedCardId, setSelectedCardId] = React.useState('nyc');
  const [collectionOpen, setCollectionOpen] = React.useState(false);
  const selectedCard =
    TRANSIT_CARDS.find((c) => c.id === selectedCardId) || TRANSIT_CARDS[0];
  ...
  // Tapping the card (not dragging) navigates to the subway map.
  const onCardClick = () => {
  ...
  const SOCIALS = [
    {
      id: 'github', label: 'GitHub', href: 'https://github.com/grqce',
      icon: (
    ...
    {
      id: 'linkedin', label: 'LinkedIn', href: 'https://www.linkedin.com/in/graceqli/',
      icon: (
        <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
    ...
    {
      id: 'email', label: 'Email', href: 'mailto:ligrace@wharton.upenn.edu',
      icon: (
        <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            <TransitCardFront card={selectedCard} />
          </div>

          {/* BACK — muted tan profile card */}
          <div
            style={{
              position: 'absolute', inset: 0,
              backfaceVisibility: 'hidden',
      ...
      <CollectionModal
        ...
        onSelect={(id) => {
          ...
          // If we're currently showing the back, flip to the new front.
